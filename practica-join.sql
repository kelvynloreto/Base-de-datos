...
-- 1er queri= todos los registros de usuarios sin importar si tiene o no cuentas creadas en el banco
...
--2do queri todas las cuentas y el tipo de cuenta al que corresponde el registro de cuenta
select accounts.account_no , clients.id , accounts.type  from clients right join accounts on clients.id = accounts.client_id  ;


--3er quieri  todas las transacciones realizadas, su monto y el tipo de transacción
...
--4to queri  todos los tipos de transacciones  y  que coincidan con todas las transacciones realizadas
...
create view v_accounts_types as select accounts.account_no , clients.id , accounts.type  from clients right join accounts on clients.id = accounts.client_id  ;

create view v_transactions_detail as select  transactions.id, transactions.amount , transactions_types.name from transactions inner join transactions_types on transactions.transaction_type  = transactions_types.id  ;
